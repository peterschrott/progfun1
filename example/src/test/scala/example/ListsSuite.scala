...
class ListsSuite {
  ...
  @Test def `sum of negative & positive numbers`: Unit = {
    assertEquals(3, sum(List(-1, -2, 6)))
  }

  @Test def `sum of empty list`: Unit = {
    assertEquals(0, sum(List.empty))
  }

  @Test def `sum of list with 1 item`: Unit = {
    assertEquals(10, max(List(10)))
  }

  @Test def `max of negative numbers`: Unit = {
    assertEquals(-1, max(List(-1, -2, -10)))
  }

  @Test def `max of negative & positive numbers`: Unit = {
    assertEquals(6, max(List(-1, -2, 6)))
  }
  ...
  @Test def `max of list with 1 item`: Unit = {
    assertEquals(10, max(List(10)))
  }
